...
public class MuPDFCore
{
	...
	static {
        ...
        try {
            ...
            String libraryPath = YoungPioneerMain.mReference.getApplicationInfo().nativeLibraryDir+"/libmupdf_java.so";
            System.out.println(libraryPath);
            System.load(libraryPath);
        }
        catch (UnsatisfiedLinkError e)
        {
            try{System.loadLibrary("mupdf_java");
            }
            catch (UnsatisfiedLinkError eh)
            {
                new AlertDialog.Builder(YoungPioneerMain.mReference)
                        .setPositiveButton(android.R.string.ok, null)
                        .setMessage("Libraries could not be loaded \n "+eh.toString()).show();
                eh.printStackTrace();

             //   YoungPioneerMain.mReference.getApplicationInfo()
            }
            //String libraryPath = YoungPioneerMain.mReference.getApplicationInfo().nativeLibraryDir;
           // System.load(libraryPath);
        }
