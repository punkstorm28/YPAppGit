...
//This method is somehow never getting into being used
	public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
			float velocityY) {
		Log.i("Motion","Something before mScaling line");
		if (mScaling||mScrollDisabled) {
			Log.i("Motion","Something with mScaling line");
			return true;
		}
		View v = mChildViews.get(mCurrent);
		if (v != null) {
    ...
    //1.5 used to be 2, changed for the angle
	private static int directionOfTravel(float vx, float vy) {
		Log.i("Motion","DirectionCheck");
